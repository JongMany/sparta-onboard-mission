...
const AccordionItem = ({title, description, projectName, detailInformation}: Props) => {
  ...
  return (
      ...
      <div
          className={"w-full min-w-[328px] bg-white border-[1px] border-[rgb(228,235,240)] shadow-accordion-shadow max-w-[600px] lg:max-w-[820px] py-[16px] px-[20px] lg:py-[20px] lg:px-[24px] rounded-[12px] break-keep cursor-pointer"}>
        <div className={"flex w-full items-center gap-[20px] lg:gap-[40px]"} onClick={toggleDetailInformation}>
          <div className={"w-full"}>
            <p className={"font-[600] text-[14px] leading-[22px] lg:text-[16px] lg:leading-[26px] text-[rgb(20,22,23)]"}>{title}</p>
            <p className={"font-[400] text-[13px] leading-[21px] lg:text-[14px] lg:leading-[22px] pt-[4px] whitespace-pre-wrap text-[rgb(58,62,65)]"}>{description}</p>
            {projectName && <div
